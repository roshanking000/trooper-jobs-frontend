const RoundedButton = ({ children, clickFunc, margin, padding, width, height, attrib, justify }) => {
  return (
    <button className={`${width == undefined ? '' : width} ${height == undefined ? '' : height} sm:relative sm:inline-flex items-center p-[2px] mt-1 text-lg font-semibold leading-6 ${margin == undefined ? '' : margin} ${attrib == undefined ? '' : attrib} text-primary rounded-full bg-grey backdrop-blur-lg hover:shadow-button-hover group hover:bg-gradient-to-r hover:from-[#F5A55B] hover:via-[#8B65F6] hover:to-[#2F40FF]`}
      onClick={clickFunc}>
