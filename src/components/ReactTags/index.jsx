...
class ReactTags extends Component {
  static propTypes = {
    ...
    offsetinput: PropTypes.string,
    setCurPreset: PropTypes.func
  };
  ...
  constructor(props) {
    ...
    const { suggestions, presets } = props;
    this.state = {
      suggestions,
      query: '',
      isFocused: false,
  ...
  handleDelete(index, event) {
    ...
    if (currentTags[index].is_preset) {
      let { presets, setCurPreset } = this.props;
      presets.push(currentTags[index].text)
      setCurPreset(presets);
      // this.setState({
      //   presets: presets
      // })
    }
  ...
  handlePresetClick(i, preset, e) {
    ...
    //   presets: presets.filter(item => item != preset)
    // })
    this.addTag({ id: preset, text: preset, is_preset: true }, true);
  }
    ...
    if (this.props.delimiters.indexOf(e.keyCode) !== -1 && !e.shiftKey) {
      ...
      const selectedQuery =
        selectionMode && selectedIndex !== -1
          ...
          : { id: query, [this.props.labelField]: query, is_preset: false };
  ...
  getPresetItems = () => {
    let { labelField, readOnly } = this.props;
      ...
      return (
        <React.Fragment key={index}>
          <Preset
            ...
            preset={preset}
            labelField={labelField}
            onPresetClicked={this.handlePresetClick.bind(this, index, preset)}
            readOnly={readOnly}
            classNames={classNames}
          />
        </React.Fragment>
  ...
  render() {
    ...
    const presetItems = this.getPresetItems();
    ...
    return (
      <div
        ...
        <div className={ClassNames(classNames.preset, this.props.offsetinput)}>
          {presetItems}
        </div>
        <div className={ClassNames(classNames.selected, this.props.offsetinput)}>
          {tagItems}
